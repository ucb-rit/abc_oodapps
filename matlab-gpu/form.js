...
function set_available_qos() {
  ...
  const selected_partition = $("#batch_connect_session_context_slurm_partition").val();
  const allqos = assocs.filter(({ account, partition }) => (account === selected_account && partition === selected_partition))[0].qos;
